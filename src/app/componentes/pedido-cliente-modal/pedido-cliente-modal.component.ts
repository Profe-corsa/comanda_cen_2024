...
import { IonButton, IonFooter, IonButtons, IonContent, IonList, IonItem, IonToolbar, IonThumbnail, IonLabel, IonTitle, IonIcon } from "@ionic/angular/standalone";
...
@Component({
  ...
  standalone: true,
  imports: [IonIcon, IonTitle, IonLabel, IonFooter, IonButton, CommonModule, IonButtons, IonContent, IonList, IonItem, IonToolbar, IonThumbnail],
})
export class PedidoClienteModalComponent implements OnInit {
  ...
  productosAgrupados: { id: string; nombre: string; precio: number; cantidad: number; fotos: string[]; tiempoEstimado: number; tipo: string;}[] = [];
  total: number = 0;
  ...
  actualizarTotal() {
    this.total = this.productosAgrupados.reduce((acc, producto) => acc + (producto.precio * producto.cantidad), 0);
  }
